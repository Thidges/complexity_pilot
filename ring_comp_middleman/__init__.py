...
class C(BaseConstants):
    NAME_IN_URL = 'ring_comp_middleman'
...
class Player(BasePlayer):
    ...
    def get_predecessor(self, predecessor_num=1):       
        predecessor = self.id_in_group - predecessor_num
        if predecessor < 1:
            predecessor += self.subsession.players_per_group
        return predecessor
            
    def get_successor(self, successor_num=1):
        successor = self.id_in_group + successor_num
        p_per_group = self.subsession.players_per_group
        if successor > p_per_group:
            successor -= p_per_group
        return successor
